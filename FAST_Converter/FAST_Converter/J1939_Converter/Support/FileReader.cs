...
namespace J1939Converter.Support
{
    class FileReader
    {
        private static string readPath = ConfigurationManager.AppSettings["ModOutFolder"] + "\\Data.txt";
        private static string writePath = "DataLog.txt";
        ...
        public static Dictionary<string, double> ReadLatest()
        {
            ...
            if (File.Exists(readPath))
            {
                ...
                try
                {
                    if (ConfigurationManager.AppSettings["FarmSimAvailable"] == "true")
                    {
